...
export async function updateTaxonomy(data: FormData) { await requireAdmin(); const kind = text(data, 'kind'); const id = text(data, 'id'); const name = text(data, 'name'); if (!name) throw new Error('Name is required'); const slug = text(data, 'slug') || slugify(name); const description = text(data, 'description'); const typedUrl = text(data, 'imageUrl'); const uploadedImage = await uploadOptionalImage(data, 'image', `${name} ${kind} artwork`); if (kind === 'category') { const current = await prisma.category.findUnique({ where: { id } }); if (!current) throw new Error('Category not found'); const imageUrl = uploadedImage?.url ?? (typedUrl || current.imageUrl || null); await prisma.category.update({ where: { id }, data: { name, slug, description: description || current.description || 'Category description', status: status(data.get('status')), imageUrl } }); } else { const current = await prisma.tag.findUnique({ where: { id } }); if (!current) throw new Error('Tag not found'); await prisma.tag.update({ where: { id }, data: { name, slug } }); } invalidate(); }

...
export async function submitPrompt(data: FormData) { const session = await getCurrentUser(); if (!session?.user.id) redirect('/login?callbackUrl=/submit'); const title = String(data.get('title') ?? '').trim(); const prompt = String(data.get('prompt') ?? '').trim(); const description = String(data.get('description') ?? '').trim(); const category = String(data.get('category') ?? '').trim(); const style = String(data.get('style') ?? '').trim(); if (title.length < 4 || prompt.length < 20 || description.length < 20 || !category || !style) throw new Error('Complete the required submission fields.'); await prisma.submission.create({ data: { userId: session.user.id, title, prompt, description, category, style, tags: String(data.get('tags') ?? '').split(',').map((tag) => slugify(tag)).filter(Boolean) } }); redirect('/submissions'); }

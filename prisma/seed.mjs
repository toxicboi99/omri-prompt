...
const styles = ['Cinematic', 'Realistic', 'Studio', 'Vintage', 'Moody', 'Minimal', 'Editorial', 'Black & White', 'Street', 'Luxury', 'Fashion', 'Dramatic'];
...
async function main() {
  ...
  const styleRows = await Promise.all(styles.map((name) => prisma.style.upsert({
    where: { slug: slugify(name) },
    update: { name },
    create: { name, slug: slugify(name), description: `A ${name.toLowerCase()} visual direction for AI image prompts.` },
  })));
  ...
  await Promise.all(Array.from({ length: 100 }, (_, index) => {
    ...
    const style = styleRows[index % styleRows.length];
    const title = `${style.name} ${category.name} Portrait ${number}`;
    return prisma.prompt.upsert({
      where: { slug: `${slugify(style.name)}-${slugify(category.name)}-portrait-${number}` },
      update: { title, categoryId: category.id, styleId: style.id, status: 'PUBLISHED' },
      create: {
        title,
        slug: `${slugify(style.name)}-${slugify(category.name)}-portrait-${number}`,
        prompt: `${style.name} AI photo of a confident ${category.name.toLowerCase()} subject, intentional composition, natural texture, precise lighting, high detail, 4:5 aspect ratio`,
        description: `A ready-to-copy ${style.name.toLowerCase()} prompt created for ${category.name.toLowerCase()} imagery.`,
  ...
  console.log('Seeded 1 admin, 20 categories, 12 styles, 10 tags, 100 prompts, AI tools, and editorial articles.');
}

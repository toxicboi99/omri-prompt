...
export async function getRelatedPrompts(prompt: PromptWithRelations) { return prisma.prompt.findMany({ where: { status: 'PUBLISHED', id: { not: prompt.id }, OR: [{ categoryId: prompt.categoryId }, { styleId: prompt.styleId }] }, include: promptInclude, take: 4, orderBy: [{ copyCount: 'desc' }, { createdAt: 'desc' }] }); }
